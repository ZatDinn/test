...
def merge_csv():
    with pd.ExcelWriter(variables.excel_path, engine='openpyxl') as writer:
        for filename in os.listdir(variables.directory):
            if filename.endswith('.csv'):
                file_path = os.path.join(variables.directory, filename)
                try:
